...
const setMove = (player, gameId, futureMove) =>
  new Promise((res, rej) =>
    isNotWaitingGame(gameId)
      ...
      .then(
        futureGameState =>
          futureGameState.gameState.winner !== 0
            ? res(
                updateGameMessage(
                  `The winner is ${futureGameState.gameState.winner}`,
                  gameId
                )
                  .then(() => gameEnded(gameId))
                  .then(() => updateGameState(futureGameState, gameId))
              )
            : updateGameState(futureGameState, gameId)
      )
...
const gameEnded = gameId =>
  new Promise((res, rej) =>
    getPlayers(gameId)
      .then(players =>
        setCurrentlyPlaying(players["playerOne"], 0).then(() =>
          setCurrentlyPlaying(players["playerTwo"], 0)
        )
      )
      .then(() => res())
      .catch(err => rej(err))
        ...
        gameState =>
          new Promise((res1, rej1) =>
            admin
              ...
              .update({
                playerTurn: gameState.playerTurn === "p1" ? "p2" : "p1"
              })
              .then(() => res1())
              .catch(err => rej1(err))
          )
...
const leaveGame = (gameId, playerId) =>
  new Promise((res, rej) =>
    getCurrentGameState(gameId)
      ...
      .then(winner =>
        updateGameMessage(`${winner.userName} is the winner`, gameId).then(() =>
          setWinner(winner, gameId)
        )
      )
...
const switchPlayer = (gameId, player) => {
  let winner = 0;
  getCurrentGameState(gameId).then(gameState => {
...
const isWinnerMove = (futureGameState, gameId, player) => {
  const winner =
    futureGameState.action !== "PAWN_MOVED"
      ? 0
      : (futureGameState.location.X === 8 &&
          futureGameState.gameState.playerTurn === "p2") ||
        (futureGameState.location.X === 0 &&
          futureGameState.gameState.playerTurn === "p1")
        ? player.email
        : 0;
...
const isValidMove = (currentGameState, futureMove) => {
  if (!validData) return Promise.reject("Invalid Data");
  ...
  switch (futureGameState["action"]) {
    case "PAWN_MOVED":
      ...
      futureGameState.gameState.availableWalls =
        currentGameState.availableWalls;

      let valid = computePotentialPawn(currentGameState).filter(d => {
        return (
          d.X === futureGameState["location"].X &&
          d.Y === futureGameState["location"].Y
        );
      });
      if (valid.length === 1) {
        futureGameState.gameState.pawns[futureGameState.gameState.playerTurn] =
          currentGameState.pawns[futureGameState.gameState.playerTurn];
        futureGameState.gameState.pawns[currentGameState.playerTurn] =
          futureMove.location;
        futureGameState.gameState.history.push(
          `Pawn ${futureMove.player} is moved to location ${
            futureGameState["location"].X
          },${futureGameState["location"].Y}`
        );
    ...
    case "WALL_ADDED":
      ...
      if (result["result"]) {
        if (currentGameState.walls) {
          if (currentGameState.walls.H)
            futureGameState.gameState.walls.H = JSON.parse(
              JSON.stringify(currentGameState.walls.H)
            );
          if (currentGameState.walls.V)
            futureGameState.gameState.walls.V = JSON.parse(
              JSON.stringify(currentGameState.walls.V)
            );
        }

        futureGameState.gameState.walls[futureMove.location[1]].push(
          futureMove.location[0]
        );
        if (futureMove.location[1] === "V")
          futureGameState.gameState.walls[futureMove.location[1]].push({
            X: futureMove.location[0].X,
            Y:
              futureMove.location[0].Y +
              (futureMove.location[0].Y === 8 ? -1 : 1)
          });
        else
          futureGameState.gameState.walls[futureMove.location[1]].push({
            X:
              futureMove.location[0].X +
              (futureMove.location[0].X === 8 ? -1 : 1),
            Y: futureMove.location[0].Y
          });

        futureGameState.gameState.availableWalls[
          futureGameState.gameState.playerTurn
        ] =
          currentGameState.availableWalls[futureGameState.gameState.playerTurn];
        futureGameState.gameState.availableWalls[currentGameState.playerTurn] =
          currentGameState.availableWalls[currentGameState.playerTurn] - 1;
        futureGameState.gameState.history.push(
          `${futureMove.location[1]} Wall is added by ${futureMove.player} in ${
            futureGameState["location"][0].X
          },${futureGameState["location"][0].Y}`
        );
...
const computePotentialPawn = gameState => {
  ...
  let bottom = gameState.walls.H.filter(
    d => gameState.pawns[player].X === d.X && gameState.pawns[player].Y === d.Y
  );
  let top = gameState.walls.H.filter(
    d =>
      gameState.pawns[player].X === d.X && gameState.pawns[player].Y - 1 === d.Y
  );
  let right = gameState.walls.V.filter(
    d => gameState.pawns[player].X === d.X && gameState.pawns[player].Y === d.Y
  );
  let left = gameState.walls.V.filter(
    d =>
      gameState.pawns[player].X - 1 === d.X && gameState.pawns[player].Y === d.Y
  );

  if (bottom.length === 0)
    potentialPawn.push({
      X: gameState.pawns[player].X,
      Y: gameState.pawns[player].Y + 1
    });
  if (top.length === 0)
    potentialPawn.push({
      X: gameState.pawns[player].X,
  ...
  if (right.length === 0)
    potentialPawn.push({
      X: gameState.pawns[player].X + 1,
      Y: gameState.pawns[player].Y
    });
  if (left.length === 0)
    potentialPawn.push({
      X: gameState.pawns[player].X - 1,
  ...
  potentialPawn = potentialPawn.filter(
    d =>
      !(
        d.X === gameState.pawns[otherPawn].X &&
        d.Y === gameState.pawns[otherPawn].Y
      )
  );
...
const verifySelectedWall = (gameState, futureGameState) => {
  ...
  if (orientation === "V")
    newSetOfWalls[orientation].push({
      X: wall.X,
      Y: wall.Y + (wall.Y === 8 ? -1 : 1)
    });
  else
    newSetOfWalls[orientation].push({
      X: wall.X + (wall.X === 8 ? -1 : 1),
      Y: wall.Y
    });
  ...
  return {
    newSetOfWalls: newSetOfWalls,
    result: !isP1Blocked && !isP2Blocked && validWall
  };
};
...
const isWallNotOverlap = (gameState, wall, orientation) => {
  ...
  if (gameState["walls"][orientation]) {
    ...
    if (wallElement.length > 0) return false;
  }
  ...
  if (!gameState["walls"][orientation === "H" ? "V" : "H"]) return true;
  ...
  if (orientation === "H") {
    ...
    wallElement = gameState["walls"]["V"].filter(d => {
      // top wall exists?
      if (!(d.X === wall.X + lastColumn && d.Y === wall.Y)) return false;
    ...
    wallElement = gameState["walls"]["H"].filter(d => {
      // left wall exists?
      if (!(d.X === wall.X && d.Y === wall.Y + lastRow)) return false;
      // right wall exists?
...
const checkPathExistence = (gameState, player, walls) => {
  ...
  for (let x = 0; x < 9; x++) {
    for (let y = 0; y < 9; y++) {
      ...
      if (x !== 0) {
        ...
        if (left.length === 0) neighbors[x - 1 + "," + y] = 1;
      } else {
        if (player === "p2") neighbors["dummy"] = 1;
      }
      if (x !== 8) {
        let right = walls.V.filter(d => x === d.X && y === d.Y);
        if (right.length === 0) neighbors[x + 1 + "," + y] = 1;
      } else {
        if (player === "p1") neighbors["dummy"] = 1;
      }
      if (y !== 0) {
        let top = walls.H.filter(d => x === d.X && y - 1 === d.Y);
        if (top.length === 0) neighbors[x + "," + (y - 1)] = 1;
      }
      if (y !== 8) {
        let bottom = walls.H.filter(d => x === d.X && y === d.Y);
        if (bottom.length === 0) neighbors[x + "," + (y + 1)] = 1;
      }
  ...
  let foundRoute = route.path(
    `${gameState.pawns[player].X},${gameState.pawns[player].Y}`,
    "dummy"
  );
...
const validData = (currentGameState, futureMove) => {
  ...
  switch (futureMove["action"]) {
    case "PAWN_MOVED":
      ...
      if (
        typeof futureMove.location.X !== "number" ||
        typeof futureMove.location.Y !== "number"
      )
        return false;
    ...
    case "WALL_ADDED":
      ...
      if (
        typeof futureMove.location[0].X !== "number" ||
        typeof futureMove.location[0].Y !== "number"
      )
        return false;
