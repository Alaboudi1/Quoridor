...
export class drawBoard {
    ...
    constructor() {
        ...
        for (let i = 0; i < 8; i++)
            for (let j = 0; j < 9; j++)
                this.gapsV.push({X: i, Y: j});

        this.gapsH = [];
        for (let i = 0; i < 9; i++)
            for (let j = 0; j < 8; j++)
                this.gapsH.push({X: i, Y: j});
        ...
        this.squaresGroup = this.svg.append('g')
        ...
        this.gapsGroup = this.svg.append('g')
            .attr('transform', `translate(${this.dimensions.margin},${this.dimensions.margin})`);
        this.pawnsGroup = this.svg.append('g')
            .attr('transform', `translate(${this.dimensions.margin},${this.dimensions.margin})`);
    ...
    subscribe() {
        ...
        // [newSetOfWalls]
        PubSub.subscribe("REFRESH_WALLS", (msg, data) => { // TODO remove
            this.walls = data[0];
            this.drawWalls();
        });
    ...
    drawSquares() {
        ...
        this.squaresGroup.selectAll('.squares')
            .data(this.squares).enter()
            .append('rect')
    ...
    drawBoardLabels() {
        ...
        this.labelsGroup.selectAll('.tempLabels')
            .data(this.squares)
            .enter()
            .append('text')
        ...
        this.labelsGroup.selectAll('.tempLabelsGapV').data(this.gapsV)
            .enter()
            .append('text')
            .classed('tempLabelsGapV', true)
            .attr('x', (d) => (d.X * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength)
            .attr('y', (d) => (d.Y * this.dimensions.gridWidthLength + 15))
            .text((d) => `${d.X},${d.Y}`);

        // horizontal

        this.labelsGroup.selectAll('.tempLabelsGapH').data(this.gapsH)
            .enter()
            .append('text')
            .classed('tempLabelsGapH', true)
            .attr('x', (d) => (d.X * this.dimensions.gridWidthLength + 15))
            .attr('y', (d) => (d.Y * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength + 5)
            .text((d) => `${d.X},${d.Y}`);
    ...
    drawPawns() {
        ...
        this.pawnsGroup.selectAll('.pawn1').data([this.pawns['p1']])
            .enter().append('circle')
            .classed('pawn pawn1', true)
            .attr('id', 'pawn1')
            .attr('cx', (d) => (d.X * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength / 2)
            .attr('cy', (d) => (d.Y * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength / 2)
            ...
            .on('click', function () {
                ...
                if (!isSelected)
                    self.displayPotentialPawn();
                else
                    self.svg.selectAll('.potentialPawn').classed('potentialPawn', false);
            });

        this.pawnsGroup.selectAll('.pawn2').data([this.pawns['p2']])
            .enter().append('circle')
    ...
    drawGaps() {
        ...
        this.gapsGroup.selectAll('.gapV').data(this.gapsV)
            .enter()
            .append('rect')
        ...
        this.gapsGroup.selectAll('.gapH').data(this.gapsH)
            .enter()
            .append('rect')
            .classed('gapH gap', true)
            .attr('x', (d) => (d.X * this.dimensions.gridWidthLength))
            .attr('y', (d) => (d.Y * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength)
    ...
    drawWalls() {
        ...
        this.gapsGroup.selectAll('.wallV').data(this.walls['V'])
            .enter()
            .append('rect')
            .classed('wallV wall', true)
            .attr('x', (d) => (d.X * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength)
            .attr('y', (d) => (d.Y /*+ ((d.Y === 8) ? -1 : 0)*/) * this.dimensions.gridWidthLength)
            .attr('width', this.dimensions.gapWidth)
            .attr('height', this.dimensions.gapLength /*+ this.dimensions.gridWidthLength*/);

        // horizontal

        this.gapsGroup.selectAll('.wallH').data(this.walls['H'])
            .enter()
            .append('rect')
            .classed('wallH wall', true)
            .attr('x', (d) => ((d.X /*+ ((d.X === 8) ? -1 : 0)*/) * this.dimensions.gridWidthLength))
            .attr('y', (d) => (d.Y * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength)
            .attr('width', this.dimensions.gapLength /*+ this.dimensions.gridWidthLength*/)
            .attr('height', this.dimensions.gapWidth);
    ...
    displayPotentialWall(d, orientation) {
        ...
        if (orientation === 'V') {
            ...
            this.svg.select('#hoveredWall')
                .attr('x', () => (d.X * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength)
                .attr('y', () => (d.Y + ((d.Y === 8) ? -1 : 0)) * this.dimensions.gridWidthLength)
                .attr('width', this.dimensions.gapWidth)
                .attr('height', this.dimensions.gapLength + this.dimensions.gridWidthLength)
            ...
            this.svg.select('#hoveredWall')
                .attr('x', () => ((d.X + ((d.X === 8) ? -1 : 0)) * this.dimensions.gridWidthLength))
                .attr('y', () => (d.Y * this.dimensions.gridWidthLength) + this.dimensions.squareWidthLength)
                .attr('width', this.dimensions.gapLength + this.dimensions.gridWidthLength)
                .attr('height', this.dimensions.gapWidth)
                .style('fill-opacity', 1);
        }
    ...
    displayPotentialPawn() {
        ...
        this.svg.selectAll('.squares')
            .filter((d) => this.potentialPawn.find((dd) => (d.X === dd.X) && (d.Y === dd.Y)) !== undefined)
