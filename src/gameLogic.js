...
export class gameLogic {
  ...
  computePotentialPawn() {
    ...
    let bottom = this.walls.H.filter(
      d => this.pawns[player].X === d.X && this.pawns[player].Y === d.Y
    );
    let top = this.walls.H.filter(
      d => this.pawns[player].X === d.X && this.pawns[player].Y - 1 === d.Y
    );
    let right = this.walls.V.filter(
      d => this.pawns[player].X === d.X && this.pawns[player].Y === d.Y
    );
    let left = this.walls.V.filter(
      d => this.pawns[player].X - 1 === d.X && this.pawns[player].Y === d.Y
    );

    if (bottom.length === 0)
      this.potentialPawn.push({
        X: this.pawns[player].X,
        Y: this.pawns[player].Y + 1
      });
    if (top.length === 0)
      this.potentialPawn.push({
        X: this.pawns[player].X,
    ...
    if (right.length === 0)
      this.potentialPawn.push({
        X: this.pawns[player].X + 1,
        Y: this.pawns[player].Y
      });
    if (left.length === 0)
      this.potentialPawn.push({
        X: this.pawns[player].X - 1,
    ...
    this.potentialPawn = this.potentialPawn.filter(
      d => !(d.X === this.pawns[otherPawn].X && d.Y === this.pawns[otherPawn].Y)
    );
  ...
  verifySelectedWall(wall, orientation) {
    ...
    if (this.availableWalls[this.playerTurn] === 0) return { result: false };
  ...
  checkPathExistence(player, walls) {
    ...
    let foundRoute = route.path(
      `${this.pawns[player].X},${this.pawns[player].Y}`,
      "dummy"
    );
