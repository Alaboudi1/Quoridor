  ...
  subscribe() {
    ...
    PubSub.subscribe("gameChange", (meg, data) => {
      ...
      if (data["gameStatus"]["walls"]) {
        this.walls.H = data["gameStatus"]["walls"]["H"]
          ? data["gameStatus"]["walls"]["H"]
          : [];
        this.walls.V = data["gameStatus"]["walls"]["V"]
          ? data["gameStatus"]["walls"]["V"]
          : [];
      } else this.walls = { H: [], V: [] };
    ...
    PubSub.subscribe("SELECTED_PAWN_POS", (msg, data) => {
      ...
      this.api
        ...
        .catch(
          err => (err.indexOf("Authorized") != -1 ? alert(err) : console.log(err))
        );
    });
    ...
    PubSub.subscribe("SELECTED_WALL", (msg, data) => {
      ...
      if (data[1] === "V")
        newSetOfWalls[data[1]].push({
          X: data[0].X,
          Y: data[0].Y + (data[0].Y === 8 ? -1 : 1)
        });
      else
        newSetOfWalls[data[1]].push({
          X: data[0].X + (data[0].X === 8 ? -1 : 1),
          Y: data[0].Y
        });
      let newAvailableWalls = JSON.parse(JSON.stringify(this.availableWalls));
      newAvailableWalls[this.playerTurn] =
        this.availableWalls[this.playerTurn] - 1;
