...
export class logic {
  constructor() {
    this.dimensions = {
      ...
      marginWidth: 15,
      pawnRadius: 15
    }; // pawnRadius = squareWidth / 2 -15

    this.squares = Array.apply(null, {
      length: Math.pow(this.dimensions.gridSize, 2)
    }).map(Number.call, Number); // 0 - 80
    this.rects = Array.apply(null, {
      length: this.dimensions.gridSize * (this.dimensions.gridSize - 1)
    }).map(Number.call, Number); // 0 - 71
    ...
    this.scaleX = d3
      ...
      .domain([0, 8])
      .range([
        this.dimensions.squareWidth / 2,
        (this.dimensions.gridSize - 0.5) * this.dimensions.gridWidth -
          this.dimensions.marginWidth / 2
      ]);
    this.scaleY = d3
      .scaleLinear()
      .domain([0, 8])
    ...
    this.squareGroup = this.svg
      .append("g")
      .attr("transform", "translate(10,10)");
    this.wallRectsGroup = this.svg
      .append("g")
      .attr("transform", "translate(10,10)");
    this.miniSquareGroup = this.svg
      .append("g")
      .attr("transform", "translate(10,10)");
  ...
  drawSquares() {
    this.squareGroup
      ...
      .attr(
        "x",
        d => (d % this.dimensions.gridSize) * this.dimensions.gridWidth
      )
      .attr(
        "y",
        d =>
          Math.floor(d / this.dimensions.gridSize) * this.dimensions.gridWidth
      )
  ...
  drawWallRect() {
    ...
    this.hoverRect = this.wallRectsGroup.append("rect").attr("id", "hoverRect");
    ...
    this.wallRectsGroup
      ...
      .attr(
        "x",
        d =>
          (d % (this.dimensions.gridSize - 1) + 1) * this.dimensions.gridWidth -
          this.dimensions.marginWidth
      )
      .attr(
        "y",
        d =>
          Math.floor(d / (this.dimensions.gridSize - 1)) *
          this.dimensions.gridWidth
      )
  ...
  drawWalls() {
    ...
    let miniSquaresH = this.walls.H.filter(
      d => this.walls.H.indexOf(d + 8) !== -1
    ); // note boundary

    let minisH = this.miniSquareGroup
      .selectAll(".miniSquareH")
      .data(miniSquaresH);
    minisH
      .enter()
      .append("rect")
    ...
    let miniSquaresV = this.walls.V.filter(
      d => this.walls.V.indexOf(d + 8) !== -1
    ); // note boundary

    let minisV = this.miniSquareGroup
      .selectAll(".miniSquareV")
      .data(miniSquaresV);
    minisV
      .enter()
      .append("rect")
  ...
  displayPotentialWall(d, orientation) {
    if (!this.isTheWallValid(d, orientation)) return;
    ...
    if (orientation === "H") {
      ...
      d3
        ...
        .attr(
          "y",
          () =>
            (d % (this.dimensions.gridSize - 1) + 1) *
              this.dimensions.gridWidth -
            this.dimensions.marginWidth
        )
        .attr(
          "x",
          () =>
            (Math.floor(d / (this.dimensions.gridSize - 1)) + diff) *
            this.dimensions.gridWidth
        )
        .attr("height", this.dimensions.marginWidth)
        .attr(
          "width",
          this.dimensions.squareWidth * 2 + this.dimensions.marginWidth
        )
        .style("fill-opacity", 0.5);
    } else if (orientation === "V") {
      // The rects are draw from right to left, top to bottom
      let diff = 0;
      if (d >= (this.dimensions.gridSize - 1) * (this.dimensions.gridSize - 1))
        // if last row
        diff = -1;
  ...
  isTheWallValid(myData, orientation) {
    ...
    let wallElement = this.wallRectsGroup
      .selectAll(".wallRect" + orientation)
      .filter(function(dd) {
        return myData === dd && d3.select(this).classed("wall");
      });
    if (wallElement.size() > 0) return false;

    // check whether there is wall on the extension of the location
    if (orientation === "H") {
      wallElement = this.wallRectsGroup
        .selectAll(".wallRectH")
        .filter(function(dd) {
          return (
            myData + self.dimensions.gridSize - 1 === dd &&
            d3.select(this).classed("wall")
          );
        });
    } else if (orientation === "V") {
      wallElement = this.wallRectsGroup
    ...
    if (orientation === "H") {
      ...
      wallElement = this.wallRectsGroup
        ...
        .filter(function(dd) {
          ...
          let neighbor = self.wallRectsGroup
            .selectAll(".wallRectV")
            .filter(function(ddd) {
  ...
  selectWall(d, orientation) {
    ...
    if (orientation === "H") {
      ...
      if (
        ...
        this.walls.H.push(d, d + this.dimensions.gridSize - 1); // if last column
      else this.walls.H.push(d, d - this.dimensions.gridSize + 1);
    } else if (orientation === "V") {
      // The rects are draw from right to left, top to bottom
      if (d < (this.dimensions.gridSize - 1) * (this.dimensions.gridSize - 1))
        // if not the last row
        this.walls.V.push(d, d + this.dimensions.gridSize - 1); // if last row
      else this.walls.V.push(d, d - this.dimensions.gridSize + 1);
    }
    this.drawWalls();
  }
